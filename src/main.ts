  ...
  private handleProcess(): void {
    ...
    if (shiftCidrText && this.lastSubnetInfo) {
      ...
      if (newCidr < 0 || newCidr > maxCidr) {
        this.showError(
          `CIDR must be between 0 and ${maxCidr} for IPv${this.lastSubnetInfo.ipVersion}`,
        );
        return;
      }
    ...
    if (checkIpText && this.lastSubnetInfo) {
      ...
      if (containment.result) {
        ...
        const innerStr = swapped
          ? `${mainInfo.network}/${mainInfo.cidr}`
          : `${checkInfo.network}/${checkInfo.cidr}`;
        const outerStr = swapped
          ? `${checkInfo.network}/${checkInfo.cidr}`
          : `${mainInfo.network}/${mainInfo.cidr}`;
...
Usable: ${subnet.usableHosts.toLocaleString()}`;
      ...
      case "Hexadecimal":
        if (subnet.ipVersion === 4) {
          const parts = subnet.network
            .split(".")
            .map((part) => parseInt(part, 10).toString(16).padStart(2, "0").toUpperCase());
